...
namespace Sample {
	...
	using namespace NationalInstruments::NI4882;	//GPIBクラスの参照
...
#pragma endregion
	...
	private: Int64 ReceiveTimeout;														//受信タイムアウト時間（ms）

	//(1)接続
	private: Boolean OpenInterface(String^ BoardID, String^ PrimaryAddress, String^ timeout) {
				 Boolean ret = false;

				 try
				 {
					 ReceiveTimeout = Convert::ToInt64(timeout) * 1000;
					 GpibDevice = gcnew Device(Convert::ToInt32(BoardID), Convert::ToByte(PrimaryAddress), 0, TimeoutValue::T10s);	//GPIBオブジェクトを作成
					 ret = true;
	...
	private: Boolean CloseInterface() {
				 ...
				 try
				 {
					 delete GpibDevice; 												//GPIBオブジェクトを廃棄
					 ret = true;
				 }
				 catch(Exception^ e)
				 {
	...
	private: Boolean ReceiveMsg(Int64 timeout) {
				 ...
				 while(true)
				 {
					 ...
					 catch(Exception^ e)
					 {
						 ...
						 if ((statusFlag & GpibStatusFlags::IOComplete) == static_cast<GpibStatusFlags>(0))
						 {
							 // IO Complete以外の例外
							 MsgBuf = "Error";
							 MessageBox::Show(e->Message);
